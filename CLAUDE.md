...
- Per-provider sender config: `FROM_EMAIL`/`FROM_NAME` are required; `REPLY_TO_EMAIL` is optional (omit to reply to from address). Set `RESEND_FROM_EMAIL`/`RESEND_FROM_NAME`/`RESEND_REPLY_TO_EMAIL` to override when Resend uses a different sending domain
